...
from peft_pretraining.modeling_llama_skip_ratio import LlamaForCausalLM
...
def parse_args(args):
    ...
    parser.add_argument("--skip_head_ratio", type=float, default=0.5) # 跨层头数的比例，越大参数越少
...
def evaluate_model(model, preprocess_batched, pad_idx, global_rank, world_size, device, batch_size):
    ...
    val_data = datasets.load_dataset("/huggingface/c4/en", split='validation', streaming=True, cache_dir="/GLOBALFS/gbu_cinco_2/zy/WZT/cache")
...
def main(args):
    ...
    data = datasets.load_dataset("/huggingface/c4/en", split='train', streaming=True)
    ...
    model_config.skip_head_ratio = args.skip_head_ratio
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        if global_rank == 0:
            ...
            logger.info(
            f"Step {global_step} | Update {update_step} | Loss {loss.item():.4f} | LR {lr:.6g} | "
            f"Tokens {tokens_seen} | Throughput: {tokens_in_update/update_time:.2f} tok/s, "
            f"{args.total_batch_size/update_time:.2f} ex/s, {batches_in_update/update_time:.2f} batch/s"
        )
            with open(os.path.join(args.save_dir, "training_log.txt"), "a") as f:
                json.dump({
                    "Loss": loss.item(),
                    "Step": global_step,
                    "Tokens": tokens_seen,
                    "Update": update_step,
                    "Throughput": tokens_in_update/update_time,
                }, f)
                f.write("\n")
        update_time = time.time()
